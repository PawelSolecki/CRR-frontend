...
describe('JobOfferForm', () => {
  ...
  test('shows loading state during form submission', () => {
    ...
    vi.mocked(useNavigation).mockImplementation(() => ({ state: 'submitting' }));

    renderJobOfferForm();

    expect(screen.getByRole('button', { name: /scraping job offer/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /scraping job offer/i })).toBeDisabled();
