...
describe('Header - Essential Tests', () => {
  ...
  test('creates correct navigation links', () => {
    ...
    expect(screen.getByRole('link', { name: /1.*Upload CV/i })).toHaveAttribute('href', '/upload-cv')
    expect(screen.getByRole('link', { name: /2.*Enhance Tags/i })).toHaveAttribute('href', '/enhance-tags')
    expect(screen.getByRole('link', { name: /3.*Job Offer/i })).toHaveAttribute('href', '/job-offer')
  })
  ...
  test('step progression logic works correctly', () => {
    ...
    const scenarios = [
      { currentStep: 1, expectedStates: { 1: 'current', 2: 'upcoming', 3: 'upcoming' } },
      { currentStep: 2, expectedStates: { 1: 'completed', 2: 'current', 3: 'upcoming' } },
      { currentStep: 3, expectedStates: { 1: 'completed', 2: 'completed', 3: 'current' } },
      { currentStep: 10, expectedStates: { 1: 'completed', 2: 'completed', 3: 'completed' } },
    ]
    ...
    scenarios.forEach(({ currentStep, expectedStates }) => {
      ...
      Object.entries(expectedStates).forEach(([stepId, expectedState]) => {
        const stepTitle = mockSteps.find(s => s.id === parseInt(stepId))?.title
        const stepLink = screen.getByRole('link', { name: new RegExp(`${stepId}.*${stepTitle}`, 'i') })
